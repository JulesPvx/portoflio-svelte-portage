<script lang="ts">
    ...
    onMount(() => {
        ...
        const stickyDiv = document.querySelector('div.sticky');
        const aboutContainer = document.getElementById('about-container');
        const footer = document.getElementById('footer');

        if (stickyDiv && aboutContainer && footer) {
            const stickyHeight = stickyDiv.offsetHeight;
            aboutContainer.style.transform = `translateY(-${stickyHeight}px)`;
            footer.style.transform = `translateY(${stickyHeight * 4}px)`;
        }

        // Function to set the height
        const adjustSectionHeight = () => {
            const svgHeight = svgElement.getBoundingClientRect().height;
            sectionElement.style.height = `${svgHeight + 700}px`;
        };
        ...
        function updateCustomShapePosition() {
            ...
            const nextPoint = path.getPointAtLength(shapePosition + 1); // Get the next point to calculate the angle

            // Calculate the angle between the current point and the next point
            const angle = Math.atan2(nextPoint.y - point.y, nextPoint.x - point.x) * (180 / Math.PI);

            // Adjust SVG position by subtracting half of its width and height to center it
            const svgWidth = 90;
            const svgHeight = 41;
            const adjustX = point.x - svgWidth / 2;
            const adjustY = point.y - svgHeight / 2; // Additional -10 for further alignment adjustment

            // Apply translation and rotation to the custom shape
...
<section class="md:pt-96 pt-48 pb-96 relative content w-full">
    ...
    <div class="sticky w-fit z-10" id="toc">
        <TOC items={items}/>
    </div>

    <div id="about-container">
        <svg class="w-full absolute" fill="none"
             id="road" viewBox="0 0 1440 3878" xmlns="http://www.w3.org/2000/svg">
            <path d="M1464.5 62.5001C1464.5 62.5001 522.887 -115.342 455 313C381.512 776.686 1250.29 838.575 1257 1308C1265.4 1896.06 958.5 1528.5 707.5 2184.5C505.68 2711.97 2361.5 3853 -47 3853"
                  id="curve"
        ...
        <!-- CARD - FREELANCE -->
        <div class="absolute right-0 translate-x-[-5vw] translate-y-[85vw]">
            <div class="image w-[16vw] relative">
        ...
        <div class=" absolute right-0 translate-x-[-40vw] translate-y-[257vw] rotate-[-10deg] flex flex-row gap-2 items-start">
            <svg class="svg rotate-[10deg] !w-[20vw]" fill="none" height="45" viewBox="0 0 315 45"
                 width="315" xmlns="http://www.w3.org/2000/svg">
